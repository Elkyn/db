...
echo -e "${BLUE}🚀 Continuing Elkyn DB Release Process${NC}"
echo ""

# Get the version from VERSION file
if [[ ! -f VERSION ]]; then
    echo -e "${RED}VERSION file not found. Run ./update_version.sh first${NC}"
    exit 1
fi
...
echo -e "${BLUE}Continuing release for version: $VERSION${NC}"
...
echo ""
echo -e "${BLUE}Building and creating GitHub release${NC}"
./create_release.sh "$VERSION_WITH_V"

# Publish npm package
echo ""
echo -e "${BLUE}Publish npm package${NC}"
